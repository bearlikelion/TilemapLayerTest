...
func _ready() -> void:
	...
	fill_tilemap()
	print("Used Cells: %s" % get_used_cells().size())
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		...
		if get_cell_source_id(Vector2i(cell_coord.x, cell_coord.y)) != -1:
			set_cell(Vector2(cell_coord.x, cell_coord.y), -1, Vector2i.ZERO)
